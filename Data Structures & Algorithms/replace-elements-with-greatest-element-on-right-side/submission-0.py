class Solution:
    def replaceElements(self, arr: List[int]) -> List[int]:
        ...
        for num in range(len(arr) -1 ,-1 ,-1):
            # then we want to find our new max number between our last number and current number
            newMax = max(lastNum, arr[num])
            # then we replace the current number we're on, with the lastNum from the updated list 
            # before the loop continues to move backwards
            arr[num] = lastNum
            # then we set our lastNum to be the newMax found, since it's the last big number found 
            lastNum = newMax
