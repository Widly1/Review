class Solution:
    def removeElement(self, nums: List[int], val: int) -> int:
        ...
        for r in range(len(nums)): 
            # If the current element is one we want to KEEP; not equal to VAL
            if nums[r] != val: 
                # Move it to the front at the next available write position; which began at 0
                nums[w] = nums[r] 
                # Move the write pointer forward; this also increases the count for numbers we do want to keep in the list
                w += 1 
