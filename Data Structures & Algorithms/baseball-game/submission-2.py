class Solution:
    def calPoints(self, operations: List[str]) -> int:
        ...
        for char in range(len(operations)):
            if operations[char] == '+': # add our previous 2 numbs from the stack
                score.append(score[-1] + score[-2])
            elif operations[char] == 'D':
                score.append(score[-1] * 2) # the double of the previous score
            elif operations[char] == 'C':
                score.pop() # pop whatever was the last input into the stack
                
            else:
                score.append(int(operations[char])) # if its a number
                                            # simply push to the stack from operations
        return sum(score) #return the sum of all values into the stack

        # other version: Direct iteration over the values
        # score = []
